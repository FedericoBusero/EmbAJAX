...
const char EmbAJAXJoystick_FREE_POSITION[] =          "if (x < -1000) x=-1000;\n"
                                                      "if (x >  1000) x= 1000;\n"
                                                      "if (y < -1000) y=-1000;\n"
                                                      "if (y >  1000) y= 1000;\n";
...
public:
    ...
    void print() const override {
        ...
        EmbAJAXBase::_driver->printContent(
           ...
           "      if (id == this.touchid) {\n"
           "        x = event.changedTouches[i].clientX - this.offsetLeft;\n" 
           "        y = event.changedTouches[i].clientY - this.offsetTop+window.scrollY;\n" 
           "      }\n"
           "    }\n"
           "  } else {\n"
           "      x = event.offsetX;\n"
        ...
        EmbAJAXBase::_driver->printContent(
           ...
           "elem.addEventListener('mousemove', function(event) { this.move(event); }.bind(elem), false);\n"
           "document.addEventListener('mouseup', function(event) { this.release(event.offsetX, event.offsetY); }.bind(elem), false);\n"
           "elem.addEventListener('touchstart', function(event) { this.touchid = event.changedTouches[0].identifier; this.press(event.touches[0].clientX-this.offsetLeft, event.touches[0].clientY-this.offsetTop+window.scrollY); }.bind(elem), false);\n"
           "elem.addEventListener('touchmove', function(event) { this.move(event); }.bind(elem), false);\n"
           "elem.addEventListener('touchend', function(event) { this.release(event.changedTouches[event.changedTouches.length-1].pageX-this.offsetLeft, event.changedTouches[event.changedTouches.length-1].pageY-this.offsetTop); }.bind(elem), false);\n"
